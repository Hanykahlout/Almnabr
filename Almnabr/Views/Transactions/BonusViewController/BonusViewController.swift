...
class BonusViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        getBonusFormData()
        addNavigationBarTitle(navigationTitle: "Bonus Form")
    ...
    private func initialization(){
        ...
        NotificationCenter.default.addObserver(forName: .init("ReloadBonusData"), object: nil, queue: .main) { notify in
            self.getBonusFormData()
        }
    ...
    private func getPrviewFile(base64:String?){
        ...
        if let base64 = base64 {
            ...
            url = "form/FORM_BNS1/pr/\(transactionId)"
        }
...
extension BonusViewController{
    private func getBonusFormData(){
        showLoadingActivity()
        APIController.shard.getBonusFormData(transactionId: transactionId) { data in
            DispatchQueue.main.async {
                ...
                if let status = data.status , status{
                    ...
                    let empObj = data.form_bns1_data?.records?.first
                    ...
                    self.amountLabel.text = empObj?.bonus_amount ?? "----"
                    self.dateLabel.text = "\(empObj?.bonus_date_english ?? "--") - \(empObj?.bonus_date_arabic ?? "--")"
    ...
    private func sendApproval(){
        showLoadingActivity()
        APIController.shard.submitApproval(formType: "FORM_BNS1", transaction_request_id: transactionId, approving_status: approveButton.isSelected ? "Approve" : "Reject", note: noteTextView.text!, transactions_persons_action_code: verificationCodeTextField.text!) { data in
            DispatchQueue.main.async {
                self.hideLoadingActivity()
                if let status = data.status ,status{
                    self.getBonusFormData()
...
extension BonusViewController: UITextViewDelegate{
