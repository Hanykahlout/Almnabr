...
//  Almnabr
//
//  Created by Hany Alkahlout on 30/11/2022.
//  Copyright © 2022 Samar Akkila. All rights reserved.
//

import UIKit
import SCLAlertView
class DeductionViewController: UIViewController {
    ...
    @IBOutlet weak var reasonProblemLabel: UILabel!
    @IBOutlet weak var deductionAmountLabel: UILabel!
    @IBOutlet weak var deductionPercentageLabel: UILabel!
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        addNavigationBarTitle(navigationTitle: "Deduction Form")
    ...
    private func initialization(){
        ...
        NotificationCenter.default.addObserver(forName: .init("ReloadDeductionData"), object: nil, queue: .main) { notify in
    ...
    private func getPrviewFile(base64:String?){
        ...
        if let base64 = base64 {
            ...
            url = "form/FORM_DET1/pr/\(transactionId)"
        }
...
// MARK: - API Controller
extension DeductionViewController{
    private func getDeductionFormData(){
        showLoadingActivity()
        APIController.shard.getDeductionFormData(transactionId: transactionId) { data in
            DispatchQueue.main.async {
                ...
                if let status = data.status , status{
                    ...
                    let empObj = data.form_det1_data?.records?.first
                    ...
                    self.joiningDateLabel.text = "----"
                    self.emailLabel.text = empObj?.primary_email ?? "----"
                    self.reasonProblemLabel.text = empObj?.deduction_reason ?? "----"
                    self.deductionAmountLabel.text = empObj?.deduction_amount ?? "----"
                    self.deductionPercentageLabel.text = empObj?.deduction_percentage ?? "----"
                    self.dateLabel.text = "\(empObj?.deduction_date_english ?? "--") - \(empObj?.deduction_date_arabic ?? "--")"
    ...
    private func sendApproval(){
        showLoadingActivity()
        APIController.shard.submitApproval(formType: "FORM_DET1", transaction_request_id: transactionId, approving_status: approveButton.isSelected ? "Approve" : "Reject", note: noteTextView.text!, transactions_persons_action_code: verificationCodeTextField.text!) { data in
            DispatchQueue.main.async {
                self.hideLoadingActivity()
                if let status = data.status ,status{
                    self.getDeductionFormData()
...
extension DeductionViewController: UITextViewDelegate{
