...
class ViolationViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        getViolationFormData()
        addNavigationBarTitle(navigationTitle: "Violation Form")
    ...
    private func initialization(){
        ...
        NotificationCenter.default.addObserver(forName: .init("ReloadViolationData"), object: nil, queue: .main) { notify in
            self.getViolationFormData()
        }
    ...
    private func getPrviewFile(base64:String?){
        ...
        if let base64 = base64 {
            ...
            url = "form/FORM_VOL1/pr/\(transactionId)"
        }
...
extension ViolationViewController{
    private func getViolationFormData(){
        showLoadingActivity()
        APIController.shard.getViolationFormData(transactionId: transactionId) { data in
    ...
    private func sendApproval(){
        showLoadingActivity()
        APIController.shard.submitApproval(formType: "FORM_VOL1", transaction_request_id: transactionId, approving_status: approveButton.isSelected ? "Approve" : "Reject", note: noteTextView.text!, transactions_persons_action_code: verificationCodeTextField.text!) { data in
            DispatchQueue.main.async {
                self.hideLoadingActivity()
                if let status = data.status ,status{
                    self.getViolationFormData()
